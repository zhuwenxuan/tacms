...
/*
 * -- -------------------------------------------------------- CREATE TABLE IF
 * NOT EXISTS `think_access` ( `role_id` smallint(6) unsigned NOT NULL,
 * `node_id` smallint(6) unsigned NOT NULL, `level` tinyint(1) NOT NULL,
 * `module` varchar(50) DEFAULT NULL, KEY `groupId` (`role_id`), KEY `nodeId`
 * (`node_id`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8; CREATE TABLE IF NOT EXISTS
 * `think_node` ( `id` smallint(6) unsigned NOT NULL AUTO_INCREMENT, `name`
 * varchar(20) NOT NULL, `title` varchar(50) DEFAULT NULL, `status` tinyint(1)
 * DEFAULT '0', `remark` varchar(255) DEFAULT NULL, `sort` smallint(6) unsigned
 * DEFAULT NULL, `pid` smallint(6) unsigned NOT NULL, `level` tinyint(1)
 * unsigned NOT NULL, PRIMARY KEY (`id`), KEY `level` (`level`), KEY `pid`
 * (`pid`), KEY `status` (`status`), KEY `name` (`name`) ) ENGINE=MyISAM DEFAULT
 * CHARSET=utf8; CREATE TABLE IF NOT EXISTS `think_role` ( `id` smallint(6)
 * unsigned NOT NULL AUTO_INCREMENT, `name` varchar(20) NOT NULL, `pid`
 * smallint(6) DEFAULT NULL, `status` tinyint(1) unsigned DEFAULT NULL, `remark`
 * varchar(255) DEFAULT NULL, PRIMARY KEY (`id`), KEY `pid` (`pid`), KEY
 * `status` (`status`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8 ; CREATE TABLE IF
 * NOT EXISTS `think_role_user` ( `role_id` mediumint(9) unsigned DEFAULT NULL,
 * `user_id` char(32) DEFAULT NULL, KEY `group_id` (`role_id`), KEY `user_id`
 * (`user_id`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8;
 */
...
// 		if (C ( 'USER_AUTH_TYPE' ) != 2 && ! $_SESSION [C ( 'ADMIN_AUTH_KEY' )])
	...
	static function saveSiteList($authId = null) {
		if (null === $authId)
			$authId = $_SESSION [C ( 'USER_AUTH_KEY' )];
		$_SESSION ['_SITE_LIST'] = RBAC::getSiteList ( $authId );
	 ...
	 *         +----------------------------------------------------------
	 */
	static public function getSiteList($authId) {
		// Db方式权限数据
		$db = Db::getInstance ();
		$table = array (
	 ...
	 *         +----------------------------------------------------------
	 */
	static public function getMenu($pid=0) {
		if (empty ( $_SESSION [C ( 'ADMIN_AUTH_KEY' )] )) {
			$userwhere = "  AND user_id=" . $_SESSION [C ( 'USER_AUTH_KEY' )];
// 			$menu = D ( "AccessView" )->having ( 'pid='.$pid.' AND name != "PUBLIC" AND site_id=' . $_SESSION ['_CURRENT_SITE'] [id] . $userwhere )->order("sort")->select ();
			$menu = D("Access")->getUserMenu($_SESSION [C ( 'USER_AUTH_KEY' )],$pid);
		} else {
			$menu = D("Node")->where('pid='.$pid.' AND name != "PUBLIC" AND status=1 AND ismenu=1')->order("sort")->select();
		}
		return $menu;
	}
	// 读取模块所属的记录访问权限
...
// 		$siteid = $_SESSION ['_CURRENT_SITE'] ['id'];
		...
		$group = D("GroupUserView")->getGroupByUser($authId);
